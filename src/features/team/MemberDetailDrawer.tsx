...
import type { Member, Project, Task, TimeEntry } from '@/types';
...
export function MemberDetailDrawer({ memberId, onClose }: { memberId: string | null; onClose: () => void }) {
  ...
  const memberTasks = (tasks ?? []).filter((t) => t.assigneeIds.includes(memberId ?? '') && t.status !== 'completed');
      ...
      >
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            ...
            <Stat label="Task attivi" value={memberTasks.length} />
