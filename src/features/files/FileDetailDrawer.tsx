...
export function FileDetailDrawer({ fileId, onClose }: { fileId: string | null; onClose: () => void }) {
  ...
  const isPdf = file.mime.includes('pdf') && file.url;

  const del = async () => { await remove.mutateAsync(file.id); toast.success('File eliminato'); onClose(); };
      ...
      <Drawer
        ...
        footer={
          <>
            ...
            {file.url && <a href={file.url} download={file.name}><Button variant="secondary" size="sm"><Download className="h-4 w-4" /> Scarica</Button></a>}
          </>
        }
      >
        <div className="space-y-5">
          ...
          <div className="flex min-h-40 items-center justify-center overflow-hidden rounded-lg border border-border bg-surface-2">
            {isImage ? (
              <img src={file.url} alt={file.name} className="max-h-64 w-full object-contain" />
            ) : isPdf ? (
              <iframe src={file.url} title={file.name} className="h-64 w-full" />
            ) : (
