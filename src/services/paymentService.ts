...
export async function syncInvoiceStatus(invoiceId?: string | null): Promise<void> {
  ...
  const payments = (await db.payments.where('invoiceId').equals(invoiceId).toArray()).filter((p) => !p.deletedAt);
  ...
  if (status !== invoice.status) {
    await db.invoices.update(invoiceId, { status, updatedAt: nowISO() });
  }
}
